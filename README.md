...
| 10 | Deliver | `/evyasys:Deliver EVYA-XXXX [...] \| EP-00N [--commit]` | 💻 Dev + 🎯 Reviewer combined | **End-to-end orchestrator**: reads story + subtasks, batches clarifying questions, architecture reference scan + brainstorm, writes the code to the working tree following every loaded standard, full self code-review with auto-fix + verifier fact-check, drafts test plan + DevSummary, queues doc updates; on approval writes 4 artefacts + updates PM + notifies. **3 gates only. No git by default** — developer commits + pushes themselves. **Opt-in `--commit`** makes the hook create the feature branch + local commit (never pushed). Batch-supports multiple stories or epic IDs. | `TechBrainstorm.md`, `DevSummary.md`, `CodeReview.md`, `TestPlan.md` (+ optional local git commit with `--commit`) | **Ready for QA** |
